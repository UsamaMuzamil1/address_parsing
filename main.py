...
def create_entity_spans(df,tag_list):
    ...
    df["SubareaTag"]=df.apply(lambda row:get_address_span(address=row['Address'],address_component=row['Subarea'],label='SUBAREA'),axis=1)
  #  df["AreaTag"]=df.apply(lambda row:get_address_span(address=row['Address'],address_component=row['Area'],label='AREA'),axis=1)
    df["CityTag"]=df.apply(lambda row:get_address_span(address=row['Address'],address_component=row['City'],label='CITY'),axis=1)
    df["ProvinceTag"]=df.apply(lambda row:get_address_span(address=row['Address'],address_component=row['Province'],label='PROVINCE'),axis=1)
    df["NameTag"]=df.apply(lambda row:get_address_span(address=row['Address'],address_component=row['Name'],label='NAME'),axis=1)
...
tag_list=["StreetTag","SubareaTag", "CityTag", "ProvinceTag", "NameTag"]

###### Training dataset prep ###########
# Read the training dataset into pandas
df_train=pd.read_csv(filepath_or_buffer="./corpus/dataset/Trainnnn.csv",sep=",",dtype=str)
...
df_test=pd.read_csv(filepath_or_buffer="./corpus/dataset/Teeeesst.csv",sep=",",dtype=str)
